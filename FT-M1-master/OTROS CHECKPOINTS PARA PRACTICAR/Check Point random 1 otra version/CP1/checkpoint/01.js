...
// Se esta organizando una fiesta para inaugurar las nuevas casas pero hay gente desconocida que intenta ingresar
...
function henryParty(arr, queue = new Queue()) {
  for (let i = 0; i < arr.length; i++) {
    let {ticket, estado} = arr[i];
    ticket === 'VIP' && estado === 'Conocido' && queue.enqueue(arr[i]);
  }
